        ...
        for i in range(len(values)):
            if values.count(values[i]) == 3:
                for j in range(len(values)):
                    if i != j and values.count(values[j]) == 2:
                        return True
        return False
...
if _name_ == "_main_":
    hand = Hand()
    cards = [Card("2", "diamonds"), Card("2", "hearts"), Card("2", "clubs"), Card("3", "hearts"), Card("3", "spades")]
    [hand.add_card(card) for card in cards]
    assert hand.get_hand_type() == "full house"

    hand = Hand()
    cards = [Card("10", "diamonds"), Card("10", "hearts"), Card("10", "clubs"), Card("10", "spades"), Card("9", "diamonds")]
    [hand.add_card(card) for card in cards]
    assert hand.get_hand_type() == "four of a kind"

    hand = Hand()
    cards = [Card("Q", "diamonds"), Card("Q", "hearts"), Card("Q", "clubs"), Card("5", "spades"), Card("9", "diamonds")]
    [hand.add_card(card) for card in cards]
    assert hand.get_hand_type() == "three of a kind"

    hand = Hand()
    cards = [Card("K", "diamonds"), Card("K", "hearts"), Card("3", "clubs"), Card("5", "spades"), Card("9", "diamonds")]
    [hand.add_card(card) for card in cards]
    assert hand.get_hand_type() == "pair"

    hand = Hand()
    cards = [Card("2", "diamonds"), Card("4", "spades"), Card("5", "clubs")]
    [hand.add_card(card) for card in cards]
    assert hand.get_hand_type() is None  # No hand type with less than 5 cards