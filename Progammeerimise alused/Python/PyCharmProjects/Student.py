...
class Student:
    ...
    def is_participating(self, course: str) -> bool:
        """Check if student is enrolled in course."""
        return course in self.__courses

    def is_failing(self):
        """Is failing when average elow 1.0"""
        return self.__average_grade < 1.0

    def succeeding_students(student_list: list) -> list:
        return list(filter(lambda s: not s.is_failing(), student_list))

    def get_average_grade(student_list: list) -> list:
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    """
    return sorted(succeeding_students(student_list), key=lambda s: (-s.get_average_grade(), s.get_name()))
...
def sort_by_worst_grade(student_list: list) -> list:
    ...
    """
    return sorted(student_list, key=lambda s: (s.get_average_grade(), s.get_name()))
